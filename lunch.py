...
# PAGE CONFIG & STYLING
# =========================
st.set_page_config(page_title="UT Yard Sukapura Lunch Claim System", layout="centered")
...
with tab1:
    ...
    st.metric(label="Sisa Jatah Makan Hari Ini", value=f"{remaining} / 168")
    st.progress(remaining / 168)
    ...
    if st.button("Cek / Klaim Makan Siang"):
        if not nrp or not name:
            ...
            with st.spinner("Sedang memproses klaim makan siang kamu..."):
                ...
                st.metric("Sisa Kuota Individu", employee[2] - (0 if not get_claim_today(nrp) else 1))
...
with tab3:
    ...
    <p>Jika mengalami kendala dalam penggunaan sistem klaim makan siang, silakan hubungi tim terkait di bawah ini:</p>
    ...
    <div style="margin-top:20px;">
        <div style="background-color:#F9F9F9; border-radius:10px; padding:15px; margin-bottom:15px; box-shadow:0px 2px 6px rgba(0,0,0,0.1);">
            <h4 style="margin-bottom:5px;">Helmalya RP</h4>
            ...
            <a href="https://wa.me/6285859823983" target="_blank">
                <button style="background-color:#25D366; color:white; border:none; border-radius:6px; padding:6px 10px; margin-top:5px; cursor:pointer;">💬 Chat via WhatsApp</button>
            </a>
        </div>

    <p style="color:#666;">Tim siap membantu Anda pada jam kerja (08.00 - 17.00 WIB).</p>
    """, unsafe_allow_html=True)
