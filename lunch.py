...
def auto_reset_daily():
    ...
    if not row or row[0] != today:
        ...
        conn.commit()
        # Perlu membersihkan cache yang menggunakan data karyawan/klaim
...
def cleanup_old_claims():
    """Hapus klaim yang lebih dari 7 hari."""
    conn = get_db_connection()
...
# =========================
# PAGE CONFIG & STYLE
# ... (Sisanya kode ini tidak perlu diubah, karena fokus perbaikan ada di atas)
# =========================
...
with tab1:
    ...
    if nrp and name:
        ...
        # Jika karyawan baru, perlu di-add dulu agar bisa ditarik
        # Perlu dipanggil add_employee
        add_employee(nrp, name)
        
        # Panggil fungsi yang di-cache
        emp = get_employee(nrp)
        claimed_today = get_claim_today(nrp)
        disable_button = bool(claimed_today) or (emp and emp[2] <= 0) # Nonaktifkan jika sudah klaim atau kuota habis
    else:
        disable_button = False
    ...
    if st.button("Cek / Klaim Makan Siang", disabled=disable_button):
        ...
        if not nrp or not name:
            ...
            # Ini dilakukan karena get_employee/get_claim_today di-cache
            # Jika user baru, add_employee sudah membersihkan cache, jadi ini aman.
            emp_data = get_employee(nrp)
            claimed_today = get_claim_today(nrp)

            with st.spinner("Sedang memproses..."):
                time.sleep(0.5) # Mengurangi waktu sleep
...
with tab2:
    ...
    if admin_pass == "admin123":
        ...
        """, conn, params=(last7,))
        # conn.close() # Tidak perlu close karena menggunakan get_db_connection()
        ...
        with c1:
            if st.button("Reset Kuota Manual"):
                ...
                # Tidak perlu clear cache secara eksplisit karena auto_reset_daily
                # sudah dipanggil di awal script, yang akan menjalankan logic reset
...
# =========================
# BACKGROUND DAILY RESET (DIHAPUS UNTUK STABILITAS DI CLOUD)
# =========================
# Baris kode background thread dihapus untuk meningkatkan stabilitas memori di Streamlit Cloud.
# Logic auto_reset_daily() yang dipanggil di awal script sudah memadai untuk reset harian.
# try:
