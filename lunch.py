...
/* PERUBAHAN: Animasi utama hanya berjalan setelah splash screen */
.main > div {{
    animation: fadeIn 0.8s ease-in-out;
}}

/* =================== SPLASH SCREEN LOGIC BARU =================== */
.ut-splash-screen {{
    position: fixed;
    top: 0;
    left: 0;
...
/* 🟢 PENAMBAHAN: KEYFRAMES ANIMASI BACKGROUND UT-HERO (Ken Burns Effect) */
@keyframes kenBurns {{
    0% {{ 
...
.ut-hero {{
    ...
    animation: kenBurns 8s ease-in-out infinite alternate; /* 20s -> 8s */
}}
...
if st.session_state.get('is_initial_load', True):
    ...
    # agar transisi logo splash screen ke hero header terlihat mulus
    st.markdown(f"""
    ...
    # dan me-re-run aplikasi untuk menampilkan konten utama
...
with tab1:
    ...
    # Session state untuk pop-up sudah diinisialisasi di awal script
