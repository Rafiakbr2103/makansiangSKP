# lunch.py — Full file (integrated timezone fix: Asia/Jakarta)
...
with tab2:
    ...
    if admin_pass == "admin123":
        ...
        today_claims = pd.read_sql_query(
            f"SELECT COUNT(*) as total FROM claims WHERE claim_date = '{datetime.now(ZoneInfo('Asia/Jakarta')).date().isoformat()}'",
            conn
        ).iloc[0]['total']
...
<div style="text-align:center;padding:14px 0;color:#9CA3AF;font-size:13px;">
    © {datetime.now(ZoneInfo('Asia/Jakarta')).year} United Tractors — Aplikasi klaim makan siang
</div>
""", unsafe_allow_html=True)
